...
class Attention(nn.Module):
    ...
    def forward(self, x, width=None):
        ...
        q = self.q_pool(q, width)
        if hasattr(self.q_pool, 'stride'):
            N = 1 + (N-1) // self.q_pool.stride ** 2
        k = self.k_pool(k, width)
        v = self.v_pool(v, width)
...
class Block(nn.Module):
    ...
    def forward(self, x, width=None):
        x = self.q_pool(x, width) + self.drop_path(self.attn(self.norm1(x), width))
...
class MultiscaleVisionTransformer(nn.Module):
        ...
        for stage_idx, num_blocks in enumerate(stages):
            ...
            for blk_idx in range(num_blocks):
                ...
                if stage_idx > 0 and blk_idx == 0:
                    ...
                    q_pool = None
                k_pool = pool_op(stage_dim, stage_stride) if stage_stride > 1 else None
                v_pool = pool_op(stage_dim, stage_stride) if stage_stride > 1 else None
                out_dim = None
                if stage_idx < len(stages) -1 and blk_idx == num_blocks -1:
                    out_dim = self.stage_dims[stage_idx+1]
                block = Block(dim=stage_dim, num_heads=stage_dim // 96,
                              mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                              qk_scale=qk_scale, drop=drop_rate,
                              attn_drop=attn_drop_rate,
                              drop_path=dpr[len(blocks)],
            ...
            blocks.append(nn.ModuleList(stage_blocks))
    ...
    def forward_features(self, x):
        ...
        #     x = blk(x)
        for stage_idx, stage_blks in enumerate(self.blocks):
            # print(f'stage_idx: {stage_idx}')
            for blk_idx, blk in enumerate(stage_blks):
                # print(f'block_idx: {blk_idx}')
                # print(f'W: {W}')
                if blk_idx == 1 and not isinstance(stage_blks[0].q_pool, Identity):
                    W //= 2
                    # print(f'new W: {W}')
